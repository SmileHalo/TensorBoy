...
x = tf.placeholder("float", [None, imageSize]) #定义占位符 X为输入结构是 宽度400 的输入流
W = tf.Variable(tf.zeros([imageSize,10]))  #定义运算变量 W为400*10的矩阵
b = tf.Variable(tf.zeros([10]))
y = tf.nn.softmax(tf.matmul(x,W) + b) #定义激活函数 y=x*w+b
y_ = tf.placeholder("float", [None,10]) #定义正确的输出值 占位符 类型为float 宽度为10
cross_entropy = -tf.reduce_sum(y_ * tf.log(y)) #计算交叉熵的函数 这里用于计算 正确值与估计量的偏差
...
def tensorFlowTrainning():
    ...
    sess.run(init)
    for i in range(50):
        tempInputList = random.sample(ih.get_trainning_images(),50)#读取数据集
